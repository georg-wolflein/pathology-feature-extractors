...
- **Magnification (~18-31x)**: Patches are 256×256 at 0.5 MPP (20x). Methods section: "we resized the WSIs to a standard resolution of 0.5 μm per pixel (MPP)—that is, 20× magnification" and "the images were cropped into 256 × 256-pixel tile images." Pretrained with DINOv2 using standard settings (Supplementary: "We followed the standard DINOv2 settings"). DINOv2 default config uses `global_crops_scale=(0.32, 1.0)` with `global_crops_size=224`. At scale=1.0: 20x × (224/256) = 17.5x; at scale=0.32: 20x × (224/144.8) = 30.9x.

## Hibou-B / Hibou-L

- **Magnification (~20-35x)**: The paper does not explicitly state the patch extraction magnification or pixel size. However, HistAI's SPIDER dataset (built using Hibou-L) uses 224×224 patches at 20x, and their HISTAI dataset lists "Standard magnification: 20X." The paper describes "splitting WSIs into nonoverlapping patches" without any resizing step, consistent with 224×224 extraction. Built on DINOv2 with default `global_crops_scale=(0.32, 1.0)` and `global_crops_size=224`. The paper's augmentation section lists modifications (rotation, RandStainNA, color jittering, no solarization) but does not change crop parameters. At scale=1.0: 20x × (224/224) = 20x; at scale=0.32: 20x × (224/126.7) ≈ 35.4x. Both Hibou-B and Hibou-L share the same magnification since they use the same DINOv2 training pipeline and input size.

## H-optimus-0

- **Magnification (~20-35x)**: HuggingFace model card states the model "expects images of size 224x224 that were extracted at 0.5 microns per pixel" (0.5 MPP = 20x). No separate paper exists; training details come from the GitHub README and press release. The model uses a DINOv2/iBOT self-supervised framework (references: Oquab et al., 2024 and Zhou et al., 2023). No modifications to default DINOv2 augmentation are documented, so default `global_crops_scale=(0.32, 1.0)` with `global_crops_size=224` is assumed. At scale=1.0: 20x × (224/224) = 20x; at scale=0.32: 20x × (224/126.7) ≈ 35.4x.

## GPFM

- **Magnification (~9-31x)**: Patches are 512×512 extracted at level 0 (native resolution) without standardization to a uniform resolution. Methods Section 4.1: "we employed the OpenSlide and CLAM toolkit to find all non-overlapping 512×512 patches at level 0 that contain tissues. It is worth noting that we did not scale the patches to a uniform resolution, opting instead to use the original resolution of each WSI." Training data is primarily from TCGA (120M patches, mix of 20x and 40x), GTEx (32M patches, 20x), and CPTAC (12M patches, 40x). Built on DINOv2 with `global_crops_scale=(0.32, 1.0)` and `global_crops_size=224` (Table A46 and confirmed in code: `ssl_default_config.yaml`). At 20x scale=1.0: 20 × (224/512) = 8.75x; at 40x scale=0.32: 40 × (224/289.7) = 30.9x.
...
- **Magnification (~10-40x)**: MUSK has two pretraining phases. In Phase 1 (masked pretraining on TCGA), patches are 512×512 at 10x magnification (1 MPP). Supplementary Methods: "approximately 50 million image patches, each with a size of 512 × 512 pixels at 10× magnification. We maintained the original image resolution without resizing to support training with multiple fields of view." The model input is 384×384 (Supplementary Table 16: "Image size 384×384"). A Field-of-View (FoV) augmentation (from Kang et al., ref. 69) randomly selects magnifications at 10×, 20×, and 40×. Methods section: "multiple fields of view (FoVs), which involve random magnifications at ×10, ×20 and ×40, were incorporated into the training pipeline." The FoV augmentation crops regions from the 512×512 patches at sizes corresponding to each target magnification and resizes to 384×384, giving effective magnifications of approximately 10×, 20×, and 40×. No RandomResizedCrop is described for the masked pretraining phase. Phase 2 (contrastive learning on QUILT-1M image-text pairs) uses "random resized cropping" but on web-scraped images with unknown/variable magnifications, so it does not affect the magnification calculation.
